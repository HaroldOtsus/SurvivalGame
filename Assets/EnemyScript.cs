...
public class EnemyScript : MonoBehaviour
{
    ...
    public float shootingRange;                 // Kaugus, millal vastane hakkab mängijat tulistama - Distance when the enemy will shoot at the player
    ...
    // Külgsuunalise liikumise muutujad - Strafing variables
    private bool isStrafing = false;        // Tõsi, kui vastane kasutab külgsuunalist liikumist
    private float strafeTimer = 0f;         // Külgsuunalise liikumise taimer
    private float strafeDuration = 2f;      // Külgsuunalise liikumise kestus sekundites
    private float strafeDistance = 2f;      // Külgsuunalise liikumise kaugus
    private Vector3 strafeDirection;        // Külgsuunalise liikumise suund
    ...
    void Update()
    {
        if (enemyIsAlive && !isEscaping)
        {
            ...
            else
            {
                ...
                // Kui mängija liigub vastasele liiga lähedale, siis vastane üritab selg ees eemalduda mängijast
                ...
                if (distanceToPlayer <= 4.0f)
                {
                    ...
                    // Liigub mängijast eemale tagurpidi
                ...
                // Kui mängija on vastasest kaugemal
                ...
                else
                {
                    ...
                    if (hit)
                    {
                        // Kui näeb mängijat, siis jääb seisma ja tulistab ning lisaks kasutab külgsuunalist liikumist
                        ...
                        if (hit.collider.name == "Player")
                        {
                            if (distanceToPlayer <= shootingRange)
                            {
                                ...
                                // Kontrollib külgsuunalise liikumise taimeri väärtust, kui on vähemvõrdne kui null, siis teeb uue külgsuunalise liikumise
                                // Checks strafing timer, if it is less or equal than 0, then enemy starts a new strafe
                                if (strafeTimer <= 0f)
                                {
                                    isStrafing = true;

                                    // Valib suvaliselt kas vasaku või parema suuna
                                ...
                                // Kui vastane liigub külgsuunaliselt
                                if (isStrafing)
                                {
                                    agent.isStopped = false;

                                    // Arvutab positsiooni kuhu külgsuunaliselt liikuda
                                    // Calculate the target position for strafing
                                    Vector3 strafeTargetPosition = transform.position + strafeDirection * strafeDistance;

                                    // Määrab asukoha kuhu külgsuunaliselt liikuda
    ...
    private void AttackPlayer(Vector2 direction, float distanceToPlayer)
    {
        ...
        if (hit)
        {
            // Kui näeb mängijat, siis jääb seisma ja tulistab
    ...
    private void Movement()
    {
        ...
        if (!agent.isStopped)
        {
            ...
            // Kontrollib shootingSpriteTimer'it, et tulistamise sprite püsiks natuke kauem nähtaval
        ...
        {
            // Kui vastane enam ei tulista, siis näita algset liikumis sprite'i
    ...
    // Vastane tulistab mängija suunas ja muudab sprite'i tulistamise sprite'iks
